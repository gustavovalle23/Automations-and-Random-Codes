...
def transform_to_paint_by_numbers(image_path, num_colors=5):
    ...
    # Create a white image with the same size as the resized image
    white_image = np.full_like(resized_image, (255, 255, 255), dtype=np.uint8)
    ...
    for idx, count in label_counts.items():
        ...
        cv2.drawContours(white_image, contours, -1, (0, 0, 0), 2)

    # Display the white image with contours
    cv2.imshow("Contours on White Image", white_image)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    # Save the white image with contours
    cv2.imwrite("contours_on_white_image.jpg", white_image)


# Example usage
transform_to_paint_by_numbers("input.png", num_colors=12)
